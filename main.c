...
void getInp(char *content) {
	...
	if(areMatching == 1) {
        printf("It took " GREEN "%ld" RESET " seconds for you to type the text.\n", time_spent);
        printf("You typed the text in " GREEN "%d" RESET " CPM (Characters Per Minute)\n", (int)(string_length(chars_received) / time_spent * 60));
		printf(YELLOW "[*] Congragulations! You successfully typed the desired text.\n" RESET);
	} else {
		printf("You were not able to type the desired text, would you like to try again? Press ENTER to continue, or type anything else to quit: ");
		int answer;
		answer = getYesOrNo();
        getInp(content);
	}
